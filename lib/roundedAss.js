...
async function createRoundedAss(subtitlePath, videoPath, outputPath, options = {}) {
    ...
    try {
        ...
        for (let idx = 0; idx < subtitles.length; idx++) {
            ...
            let dimensions = subtitleDimensions && subtitleDimensions[idx]
                ? subtitleDimensions[idx]
                : calculateTextDimensions(sub.text, fontName, adjustedFontSize, videoWidth);

            let { width: textWidth, height: textHeight } = dimensions;
            const maxSubtitleHeight = videoHeight * 0.33;

            // Dynamically adjust font size to fit subtitle vertically
            while ((textHeight + extendedConfig.paddingV * 2) > maxSubtitleHeight && adjustedFontSize > 10) {
                adjustedFontSize -= 2; // Reduce size gradually
                dimensions = calculateTextDimensions(sub.text, fontName, adjustedFontSize, videoWidth);
                textWidth = dimensions.width;
                textHeight = dimensions.height;
            }

            // Compute box dimensions after font adjustment
            ...
            // Determine effective radius again after adjustment
            ...
            // Position subtitles ensuring marginBottom is respected
            const yPos = videoHeight - marginBottom - halfHeight;
